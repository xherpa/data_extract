...
class Facebook_ads_data(Base):
    ...
    adset_budget = Column(String,  index=True)
    results = Column(Float)
    cost_per_result = Column(String, index=True)
    reach = Column(Float)
    impressions = Column(Float)
    amount_spend = Column(String, index=True)
    cost_per_1000_accounts_centre_accounts_reached = Column(String, index=True) #cpm
